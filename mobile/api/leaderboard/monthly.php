...
try {
    // For monthly, we aggregate daily_activity for the last 30 days.
    $sql = "
        WITH MonthlyAgg AS (
            SELECT 
                user_id,
                SUM(xp_earned) as monthly_xp,
                SUM(courses_completed) as monthly_courses,
                SUM(cases_completed) as monthly_cases,
                SUM(streak_increment) as monthly_streak
            FROM daily_activity
        ...
        RankedUsers AS (
            SELECT 
                w.user_id,
                COALESCE(w.monthly_xp, 0) as total_xp,
                COALESCE(w.monthly_streak, 0) as current_streak,
                COALESCE(w.monthly_cases, 0) as investigations_completed,
                COALESCE(w.monthly_courses, 0) as courses_completed,
                RANK() OVER (ORDER BY COALESCE(w.monthly_xp, 0) DESC) as rnk
            FROM MonthlyAgg w
        )
        SELECT 
            sub.user_id,
