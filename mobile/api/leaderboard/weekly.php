...
try {
    // For weekly, we aggregate daily_activity for the last 7 days.
    $sql = "
        WITH WeeklyAgg AS (
            SELECT 
                user_id,
                SUM(xp_earned) as weekly_xp,
                SUM(courses_completed) as weekly_courses,
                SUM(cases_completed) as weekly_cases,
                SUM(streak_increment) as weekly_streak
            FROM daily_activity
        ...
        RankedUsers AS (
            SELECT 
                w.user_id,
                COALESCE(w.weekly_xp, 0) as total_xp,
                COALESCE(w.weekly_streak, 0) as current_streak,
                COALESCE(w.weekly_cases, 0) as investigations_completed,
                COALESCE(w.weekly_courses, 0) as courses_completed,
                RANK() OVER (ORDER BY COALESCE(w.weekly_xp, 0) DESC) as rnk
            FROM WeeklyAgg w
        )
        SELECT 
            sub.user_id,
