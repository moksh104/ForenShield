...
class AuthStateNotifier extends AsyncNotifier<UserModel?> {
  ...
  Future<UserModel?> build() async {
    ...
    if (await _tokenService.isLoggedIn()) {
      ...
      if (storedUser != null) {
        AppLogger.d('[AuthStateNotifier] Found saved user in storage: ${storedUser.email}');
        return storedUser;
      }

      AppLogger.d('[AuthStateNotifier] Access token exists. Fetching current user profile...');
      final result = await _repo.getCurrentUser();
      
      if (result.isSuccess) {
        final user = (result as Success<UserModel>).data;
        await _tokenService.saveUser(user);
        AppLogger.d('[AuthStateNotifier] Current user profile fetched successfully: ${user.email}');
        return user;
      } else {
        AppLogger.w('[AuthStateNotifier] Fetching current user failed. Clearing session.');
  ...
  Future<Result<void>> login(String email, String password) async {
    ...
    if (result.isSuccess) {
      ...
      AppLogger.d('[AuthStateNotifier] Login API call succeeded! Storing tokens and user data...');
      ...
      AppLogger.d('[AuthStateNotifier] Tokens stored successfully. Updating Riverpod state to user: ${response.user.email}');
  ...
  Future<Result<void>> register(String email, String password, String displayName) async {
    AppLogger.d('[AuthStateNotifier] Initiating registration for email: $email');
    ...
    if (result.isSuccess) {
      final response = (result as Success<AuthResponseModel>).data;
      AppLogger.d('[AuthStateNotifier] Registration succeeded! Storing tokens...');
      await _tokenService.saveToken(response.accessToken);
      await _tokenService.saveRefreshToken(response.refreshToken);
      await _tokenService.saveUser(response.user);
  ...
  Future<Result<void>> verifyOtp({
    ...
    if (result.isSuccess) {
      final response = (result as Success<AuthResponseModel>).data;
      AppLogger.d('[AuthStateNotifier] OTP verification succeeded! Storing tokens...');
      await _tokenService.saveToken(response.accessToken);
      await _tokenService.saveRefreshToken(response.refreshToken);
      await _tokenService.saveUser(response.user);
